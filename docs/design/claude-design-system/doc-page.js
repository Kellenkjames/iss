  ...
  class DocPage extends HTMLElement {
    ...
    constructor() {
      ...
      this._mo = typeof MutationObserver === 'function'
        ...
        : null;
      // Header/footer height can settle after connect (web font swap,
      // slotted content reflow) with no further DOM mutation to trigger
      // the MutationObserver — a ResizeObserver on the slotted elements
      // themselves catches that final layout regardless of timing.
      this._ro = typeof ResizeObserver === 'function'
        ? new ResizeObserver(() => this._scheduleMeasure())
        : null;
      this._roTargets = new Set();
    }
    ...
    _scheduleMeasure() {
      // rAF can be paused indefinitely in a hidden/inactive preview iframe,
      // so scheduling must not depend on it firing — setTimeout still runs.
      if (this._measureTimer) return;
      this._measureTimer = setTimeout(() => { this._measureTimer = null; this._measure(); }, 0);
    }
    ...
    _measure() {
      ...
      if (this._ro) {
        for (const el of [hdr, ftr]) {
          if (el && !this._roTargets.has(el)) {
            this._ro.observe(el);
            this._roTargets.add(el);
          }
        }
      }
